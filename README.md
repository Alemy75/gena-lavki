...
**Через админку** (без SSH): страница «Резервная копия» → форма «Восстановление
из бэкапа» — загрузить `.tgz`, поставить галочку, нажать кнопку. Перезаписывает
БД и загрузки, затем применяет миграции (старый бэкап догоняется до текущей
схемы). Нужен работающий контейнер `app`; если стек мёртв — восстанавливайся
по SSH, как ниже.
